...
class MCPServerBase(ABC):
                ...
                while not self._role_monitor_stop.is_set():
                    ...
                    if role != self._last_provider_role:
                        ...
                        if role == "RW" and self._last_provider_role != "RW":
                            if not self._should_skip_indexing() and self._index_on_promotion_enabled():
    ...
    async def _background_initial_scan(self, target_path: Path) -> None:
        ...
        try:
            ...
            # opportunistic RO readers a window to access the DB.
            try:
                import os as _os
                idle_ms = int(_os.getenv("CHUNKHOUND_MCP__RW_IDLE_DISCONNECT_MS", "0") or 0)
            except Exception:
                idle_ms = 0
