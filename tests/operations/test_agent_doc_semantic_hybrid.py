...
from operations.agent_doc import (
    AgentDocMetadata,
    _build_lexical_pattern_from_query,
...
def test_build_lexical_pattern_from_query_picks_reasonable_token() -> None:
    pattern = _build_lexical_pattern_from_query("Explain the semantic architecture of this module")
    # We don't fix the exact token, but it should be a word-boundary-style regex fragment
    assert pattern is None or "\\b" in pattern


async def test_semantic_overview_with_lexical_sidecar_uses_both_sources(tmp_path: Path) -> None:
    ...
    result = await _run_semantic_overview_query(
        ...
        hyde_plan=None,
        debug_dir=debug_dir,
        lexical_sidecar=True,
    )

    # We still get a valid Agent Doc body back
    assert "# Agent Doc" in result

    # Both semantic and regex paths should have been called
    assert services.search_service.semantic_calls
    assert services.search_service.regex_calls

    # Merged debug results should include both code and test files
