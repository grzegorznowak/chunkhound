...
fast fallback when a writer holds the DB, and reset after writer exits.
"""
    ...
    with windows_safe_tempdir() as tmp:
        ...
        async def _run():
            ...
            try:
                ...
                # First attempt during conflict: return fast and set backoff
                t_start = time.time()
                ...
                # Should be fast because backoff prevents attempts (<200ms)
                assert (time.time() - t2) < 0.2
                s = await _get_stats(c_ro)
