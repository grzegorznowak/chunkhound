...
2) While an Indexer is active (holding a write connection), a Non-Indexer with
   RO_TRY_DB=1 returns quickly (falls back) for DB-backed tools (no timeouts).
...
def test_opportunistic_ro_falls_back_quickly_during_writer():
    with windows_safe_tempdir() as tmp:
        ...
        async def _run():
            ...
            try:
                # While RW is likely connected (initial scan), RO query should return fast (<= 2s) even if empty
                t0 = time.time()
                ...
                # Either results (if lucky) or fast empty fallback, but no timeout
