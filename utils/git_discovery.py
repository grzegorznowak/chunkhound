...
def _run_git_ls_files(repo_root: Path, pathspecs: list[str] | None = None) -> tuple[list[str], int, int]:
    ...
    try:
        ...
        tracked = [p for p in res.stdout.decode("utf-8", "ignore").split("\x00") if p]
    except Exception:
        tracked = []
    # Untracked, non-ignored (exclude-standard honors .gitignore + core excludes)
    others: list[str] = []
    try:
        cmd = ["git", "-C", str(repo_root), "ls-files", "--others", "--exclude-standard", "-z"]
        if pathspecs:
            cmd += ["--", *pathspecs]
        res = subprocess.run(
            cmd,
        ...
        others = [p for p in res.stdout.decode("utf-8", "ignore").split("\x00") if p]
    except Exception:
        others = []
